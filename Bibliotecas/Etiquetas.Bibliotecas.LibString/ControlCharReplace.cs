...
namespace Etiquetas.Bibliotecas.LibString
{
    public static class ControlCharReplace
    {
        public static string Execute(string data, Dictionary<string, char> chrList)
        {
            ...
            {
                int openBracketIndex = Etiquetas.Bibliotecas.LibStrBuilder.IndexOf.PosicaoKMP(result, "[", startIndex);
                int closeBracketIndex = (openBracketIndex != -1) ? Etiquetas.Bibliotecas.LibStrBuilder.IndexOf.PosicaoKMP(result, "]", openBracketIndex + 1) : -1;
